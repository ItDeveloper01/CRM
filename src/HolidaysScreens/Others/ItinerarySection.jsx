...
export default function ItinerarySection({
    ...
    //     const base =
    //         current.variant.perPaxBaseAmount || 0;

    //     const pickup =
    //         current.pickup?.ratePerPax || 0;
    ...
    const handleSearchToursByCode = async () => {
        ...
        try {
            ...
            console.log("Search Result by Code:", res.data);
        ...
        const selectedItineraries = tabs
            .map((_, index) => {
                ...
                return {
                    ...
                    pickupCity:
                        state.pickup?.pickupCity || "",

                    targetAudienceId:
                        state.targetAudienceId,
                        ...
                        <div className="flex-1">
                            ...
                            <div className={UI.sectionCard}>
                                ...
                                <div className="grid grid-cols-12 gap-4 mt-4">
                                    ...
                                    <div className="col-span-12 mt-0 flex justify-end gap-3">
        ...
        <div className={UI.tourPanel}>
          ...
          <div className={UI.tourList}>
            {current.itineraries.map(it => {
              const isSelected = current.itinerary?.itineraryId === it.itineraryId;
              return (
                <button
                  key={it.itineraryId}
                  type="button"
                  ...
                  className={`${UI.tourCard} ${isSelected ? UI.tourCardSelected : UI.tourCardNormal}`}
                >
          ...
          <div className="p-2 flex flex-wrap gap-2">
            {audienceGroups.map(([audience, variants], index) => {
              ...
              return (
                <button
                  key={audience}
                  onClick={() => updateTabState(activeTab, { activeAudience: audience })}
                  className={`px-3 h-8 rounded-full border text-xs font-medium transition-all ${selected ? color.selected : color.light}`}
                >
                  {audience}
          ...
          <div className="flex flex-wrap gap-2 content-start">
            {(audienceGroups.find(([a]) => a === activeAudience)?.[1] || [])
              ...
              .map((variant) => {
                ...
                const selected = current.variant?.variantId === variant.variantId;
                return (
                  <button
                    ...
                    className={`px-4 h-10 rounded-lg border text-sm transition-all ${selected ? color.selected : "bg-white hover:bg-gray-50"}`}
                  >
      ...
      <div className={UI.tourPanel}>
        ...
        <div className={UI.tourList}>
          {current.pickupPoints.map(point => {
            const selected = current.pickup?.pickupPointId === point.pickupPointId;
            return (
              <button
                key={point.pickupPointId}
                type="button"
                onClick={() => updateTabState(activeTab, { pickup: point })}
                className={`${UI.tourCard} ${selected ? UI.tourCardSelected : UI.tourCardNormal}`}
              >
...
<div className="border-2 border-green-200 rounded-xl bg-white shadow-sm overflow-hidden relative">
    ...
    <div className="px-4 py-3 border-b border-green-200 bg-green-50 flex items-center gap-2">
        ...
        {/* Discount visibility toggle — internal-only info, off by default */}
        <button
            type="button"
