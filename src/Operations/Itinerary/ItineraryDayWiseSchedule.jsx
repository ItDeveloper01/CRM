...
import { getEmptyItinerarydayObj } from "../Model/ItineraryModel";
...
//   return { id: uid(), dayTitle: `Day ${n} Title`, activities: [] };
// }
export function mkDay(n) {

  const obj = getEmptyItinerarydayObj();
...
export default function DayWiseSchedule({
  ...
  //Days from itineraryObj
  const days = itineraryObj.days;
    ...
    setItineraryObj(prev => ({
      ...
      days: prev.days.map((day, dayIndex) => {
        ...
        if (editingIndex !== null) {
          ...
          activities[editingIndex] = activityWithId;
      ...
      >
        ...
        <DayTabs
          ...
          onDeleteDay={(index) => {
            ...
            //   setSelectedDay(Math.max(0, selectedDay - 1));
            // }  
            //uncomment below if days deleting section not work properly 
          ...
          <div style={dayPanelStyle}>
            ...
            <div>
              {days[selectedDay].activities?.map((activity, index) => (
                ...
                >
                  {/* Time */}
                  <div style={activityTimeStyle}>{activity.activityTime}</div>

                  {/* Title + notes */}
                  <div style={{ flex: 1 }}>
                    <div style={activityTitleStyle}>{activity.activityTitle}</div>
                    {activity.activityNotes && (
                      <div style={activityNotesStyle}>{activity.activityNotes}</div>
                    )}
                  </div>
