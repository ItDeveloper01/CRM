  ...
  "Jul 2026": [
    {
      ...
      dates: "5–12 Jul",
      guide: "James D'Silva",
      cities: "Chennai, Kolkata, Bangalore",
      seats: 18,
      total: 20,
      rate: "₹42,000",
    },
  ...
  "Aug 2026": [
    {
      ...
      dates: "5–14 Aug",
      guide: "Riya Sharma",
      cities: "Kolkata, Shillong, Tawang",
      seats: 16,
      total: 18,
      rate: "₹36,000",
    },
...
function KanbanColumn({
  ...
  return (
    <div
      className={`${STYLES.colWidth} ${STYLES.colWrapper}  h-full flex flex-col`}
    >
...
*/
  ...
  const currentYear = currentDate.getFullYear(); // 1-12
  const currentMonth = currentDate.getMonth() + 1;

  // const [selectedYear, setSelectedYear] = useState(new Date().getFullYear());
  const [selectedYear, setSelectedYear] = useState(currentYear);
  // const MONTHS_ORDER = MONTHS.map(m => `${m} ${selectedYear}`);
  const MONTHS_ORDER = getMonths(selectedYear);


  //To load month according to current month
  const getInitialMonths = () => {
    ...
    // One months back
    const startMonth = Math.max(1, currentMonth - 1);
  ...
  const [visibleMonths, setVisibleMonths] = useState(getInitialMonths());

  // Active tab should be selected initially
  ...
  // Convert Month Number to Column Name
  const getMonthLabel = (month) => {
    return `${MONTH_NAMES[month - 1]} ${selectedYear}`;
  };
  // const getCacheKey = (year, status, month) => {
  //   return `${year}-${status}-${month}`;
  // };
  ...
  const handleManageFormSave = (formData) => {
    ...
    if (editingCard) {
      ...
      setData((prev) => ({
        ...prev,
        [selectedMonth]: [...(prev[selectedMonth] || []), newCard],
      }));
    }
  ...
  const handleSave = async (request) => {
    ...
    // This handlsave is used to call APi From UseItinerary.js
    ...
    try {
      ...
      // 🔄 Refetch fresh dashboard data so the new/updated itinerary
      // shows immediately, without a page reload
      await refreshDashboard();

      if (request.id) {
        // UPDATE — find which month held this itinerary and replace it
        const month = findMonth(editingItinerary);
        if (month) {
          setData((prev) => ({
            ...prev,
            [month]: prev[month].map((c) =>
              c.id === request.id ? response : c
            ),
          }));
        }
      } else {
        // CREATE — add the API's returned record to the selected month
        setData((prev) => ({
          ...prev,
          [selectedMonth]: [...(prev[selectedMonth] || []), response],
        }));
      }
      ...
      // error state is already captured inside useItinerary's `error`
    }
  ...
  // }, []);
  //group response month wise
  const groupByMonth = (rows) => {
    const grouped = {};
    debugger;
    MONTHS.forEach((m, i) => {
      grouped[`${m} ${selectedYear}`] = [];
    });

    rows.forEach((item) => {
      const monthName = MONTHS[item.month - 1];

      grouped[`${monthName} ${selectedYear}`].push(item);
    });

    return grouped;
  };

  // its working fine date 23-07-2026
  ...
  // setData(groupByMonth(response.data.data));
...
*/
    ...
    // Already loaded and no refresh requested? skip.
    if (dashboardCache[cacheKey] && !forceRefresh) {
      return;
    }
    ...
    try {
      ...
      console.log("Dashboard Fetch api response :", response);
  ...
  //helper that refreshes exactly the months currently on screen
  const refreshDashboard = async () => {
    setLoadingDashboard(true);

    const statusId = STATUS_MAPPING[activeFilter];

    const requests = visibleMonths.map((month) =>
      fetchMonth(selectedYear, month, statusId, true) // forceRefresh = true
    );
  ...
  const loadPreviousMonth = async () => {
     if (isLoadingPrevRef.current) return;      // duplicate call guard
    const smallest = Math.min(...visibleMonths);

    if (smallest === 1)
      return;

    const previous = smallest - 1;
  ...
  const prevScrollWidth = container ? container.scrollWidth : 0;
    ...
    // naya column add hone ke baad scroll position ko compensate karo
  ...
  const loadInitialMonths = async () => {
    ...
    //     //This fetch the data According to month monthwise logic 
  ...
  const handleScroll = async () => {
    ...
    if (container.scrollLeft < 150 && !isLoadingPrevRef.current) {
...
// setVisibleMonths(getInitialMonths());
...
// ]);
  ...
  return (
    <div
      className={`h-screen flex flex-col ${STYLES.pageBg} ${STYLES.pagePadding} `}
    >
      {/* ── Header ── */}
      <div className="flex items-center justify-between mb-4 shrink-0">
        ...
        <div className="flex items-center gap-2">
          ...
          <button className={STYLES.newBtn} onClick={handleNewBtn}>
            ＋ New itinerary
          </button>
        </div>
      </div>
      ...
      {loadingDashboard && (
        <div className="text-blue-700 font-medium">Loading Dashboard...</div>
      )}
      ...
      <div
        ref={boardRef}
        // className="flex-1 overflow-x-auto overflow-y-hidden pb-3 min-h-0">
        className="flex-1 min-w-0 overflow-x-auto overflow-y-hidden pb-3 min-h-0">
        <div className="flex gap-3 h-full" style={{ minWidth: "max-content" }}>
          ...
          {visibleMonths.map(month => {
            ...
            const cards = getMonthCards(month).map(mapToCard);
